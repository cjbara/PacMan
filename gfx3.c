...
/* Draw a filled in circle; uses a version of Bresenham's circle algorithm */
void gfx_fill_circle( int xCtr, int yCtr, int r )
{
   ...
   for (x = 0; x <= y; x++)
   {
	gfx_line(xCtr + x, yCtr + y,xCtr + x, yCtr - y);
        gfx_line(xCtr - x, yCtr + y,xCtr - x, yCtr - y);
        gfx_line(xCtr + y, yCtr + x,xCtr + y, yCtr - x);
        gfx_line(xCtr - y, yCtr + x,xCtr - y, yCtr - x);
...
/* Draw a filled triangle connecting (x1,y1), (x2,y2), and (x3,y3) */
void gfx_fill_triangle( int x1, int y1, int x2,int y2,int pacOrientation)
{
  switch(pacOrientation){
    case 0:
      while(x1<x2){//up
	gfx_line(x1,y1,x2,y2);
	x1++;
